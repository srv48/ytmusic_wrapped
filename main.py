...
def get_args():
    parser = argparse.ArgumentParser(description='GMusic Year Wrapper')
    parser.add_argument("file", type=str, help="Provide a Google Activity file")
    parser.add_argument("-d", "--duration", type=str, help="Provide your Last.fm API key")
    parser.add_argument("-v", "--verbose", action='store_true', help="Verbose logging")
    parser.add_argument("-y", "--year", type=str, help="Year to create a wrap for")
    return parser.parse_args()

def flags():
    args = get_args()
    global expect, verbose, duration, lastFmToken, filename
    if args.year:
        expect = args.year
    verbose = args.verbose
    duration = bool(args.duration)
    lastFmToken = args.duration
    filename = args.file
...
def gen_html_report(cursor, data, expect):
    ...
    print ("""</div><br><br><div class="row"><div class="column-1"><div class="minutes_title">Top Artists</div><div class="list">""", file = htmlreport)
    cursor.execute(
    ...
    print ("""</div></div><div class="column-2"><div class="minutes_title">Top Songs</div><div class="list">""", file = htmlreport)
